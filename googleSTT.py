...
CHANNELS = 1
RATE = 44100
THRESHOLD = 45000  # The threshold intensity that defines silence
...
def stt_google_wav(audio_fname):
    ...
    req = Request(GOOGLE_SPEECH_URL)
    req.add_header("User-Agent", hrs["User-Agent"])
    req.add_header("Content-type", hrs["Content-type"])
    print("Sending request to Google TTS")
    #print "response", response
    try:
        p = urllib.request.urlopen(req, data=flac_cont)
        response = p.read()
